...
/// <summary>
/// Sliding-window limit on uploads, partitioned by client IP. Behind an ingress, forwarded headers must be
/// configured so the partition key is the real client rather than the proxy (ADR 0008).
/// </summary>
internal sealed class UploadRateLimitPolicy(IOptions<UploadRateLimitOptions> options) : IRateLimiterPolicy<string>
{
    ...
    private static async ValueTask WriteRejectionAsync(OnRejectedContext context, UploadRateLimitOptions settings, CancellationToken cancellationToken)
    {
        ...
        await problemDetails.WriteAsync(new ProblemDetailsContext
        {
            ...
            ProblemDetails = new ProblemDetails
            {
                ...
                Detail = "The upload rate limit was exceeded. Retry after the interval in the Retry-After header.",
            },
        });
    }
}
